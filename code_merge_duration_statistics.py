    ...
    for change in total_changes:
        ...
        for message in reversed(messages):
            ...
            if "Code-Review+2" in message["message"]:
                first_review2_date = message["date"].rstrip('0').ljust(26, '0')[:26]
        ...
        merge_duration           = merge_date - first_review2_date
        ...
        print(f'{change["_number"]} took {merge_duration_formatted} from first code_review+2 to code_merge')
...
def save_result(total_changes, code_merge_duration):
    ...
    csvheader                 = ['change', 'subject', 'owner', 'merge_duration']
    tableheader               = ['change', 'owner', 'merge_duration']
    ...
    table  = [[item.get('change'), item.get('owner'), item.get('merge_duration')] for item in code_merge_duration]
